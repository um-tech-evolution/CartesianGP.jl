...
c0 = Chromosome(p0,true)  # c0 has a cache
...
@test c0.cache.number_active_nodes == 6  # set during execution
# Remove cache
c0.cache = ChromosomeCache(p0)    # Nonfunctional cache
c0.has_cache = false
@test execute_chromosome(c0) == BitString[0x6,0x8]  # Value should not change
#print_chromosome(c0)
...
c1 = Chromosome(p1,true)  # c1 has a cache
...
@test c1.cache.number_active_nodes == 8  # set during execution
@test c1.cache.interiors[1,1].active == true
@test c1.cache.interiors[2,1].active == true
@test c1.cache.interiors[3,1].active == false
@test c1.cache.interiors[4,1].active == true
@test c1.cache.interiors[5,1].active == true
# Remove cache
c1.cache = ChromosomeCache(p1)    # Nonfunctional cache
c1.has_cache = false
@test execute_chromosome(c1) == BitString[0x2,0xa]  # Value should not change
#print_chromosome(c1)
